...
### 5. Исправление line endings (только при первом запуске)
...
**`$(pwd)` не работает в Windows CMD** — использовать `%cd%` вместо `$(pwd)` или запускать команды в Git Bash.

**`run_pipeline.bash: command not found`** — выполнить `sed -i 's/\r//' run_pipeline.bash` для исправления line endings.

**Ошибка сети при скачивании моделей** — запустить пайплайн повторно, уже скачанные модели не будут скачиваться заново.

**Docker Desktop завис** — перезапустить Docker Desktop через трей и выполнить `docker build` заново, кэш сохранится.
